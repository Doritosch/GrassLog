...
export default function CategoryInput({ categories, selected, onChange, compact = false }) {
  ...
  const addNewCategory = async (name) => {
    ...
    if (existing) { handleSelect(existing.name); return }
    const formData = new FormData()
    formData.set('name', name)
    const result = await createCategory(formData)
    if (result?.error && result.error !== '이미 존재하는 카테고리예요.') { alert(result.error); return }
  ...
  const handleKeyDown = async (e) => {
    if (e.key === 'Enter' && input.trim()) { e.preventDefault(); await addNewCategory(input.trim()) }
  ...
  const dropdownContent = (
    <div className="max-h-40 overflow-y-auto">
        ...
        return (
          <div key={cat.id} className="flex items-center group">
            ...
            <button
              ...
              className="px-2 py-1.5 hover:text-red-400 opacity-0 group-hover:opacity-100 text-xs transition-all"
      ...
      {input.trim() && !localCategories.find((c) => c.name.toLowerCase() === input.trim().toLowerCase()) && (
        <button
          type="button"
          onMouseDown={async (e) => { e.preventDefault(); await addNewCategory(input.trim()) }}
          className="w-full text-left px-3 py-1.5 text-xs border-t"
          style={{ color: 'var(--text-muted)', borderColor: 'var(--border)' }}
          onMouseEnter={e => e.currentTarget.style.background = 'var(--bg-elevated)'}
          onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
        >
          <span style={{ color: 'var(--link)' }}>+ "{input.trim()}"</span> 새로 만들기
        </button>
      )}
      {filtered.length === 0 && !input.trim() && (
        <p className="px-3 py-2 text-xs" style={{ color: 'var(--text-muted)' }}>카테고리를 입력하세요</p>
      )}
    </div>
  )
  ...
  if (compact) {
    return (
      <div ref={containerRef} className="relative">
        ...
        {open && (
          <div
            className="absolute bottom-full right-0 mb-1 w-52 rounded-md shadow-lg z-20 border"
          ...
          >
            <div className="p-2 border-b" style={{ borderColor: 'var(--border)' }}>
              <input
                ...
                className="w-full border rounded px-2 py-1 text-xs placeholder-[color:var(--text-muted)] focus:outline-none focus:border-[var(--link)]"
              />
            </div>
            {dropdownContent}
          </div>
  ...
  return (
    <div ref={containerRef} className="relative flex-1">
      <div
        className="flex items-center flex-wrap gap-1 px-3 py-2 border rounded-md focus-within:border-[var(--link)] cursor-text min-h-[38px]"
      ...
      >
        {selected.map((name) => {
          ...
          return (
            <span key={name} className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-full border whitespace-nowrap"
        ...
        <input
          ...
          className="flex-1 bg-transparent placeholder-[color:var(--text-muted)] text-sm focus:outline-none min-w-[120px]"
        />
      </div>
      {open && (
        <div
          className="absolute top-full left-0 right-0 mt-1 rounded-md shadow-lg z-10 max-h-48 overflow-y-auto border"
          style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}
        >
          {filtered.map((cat) => {
            const color = getCategoryColor(cat.name)
            return (
              <div key={cat.id} className="flex items-center group">
                <button type="button" onClick={() => handleSelect(cat.name)}
                ...
                <button type="button"
                  ...
                  className="px-3 py-2 hover:text-red-400 opacity-0 group-hover:opacity-100 text-xs transition-all"
                  style={{ color: 'var(--text-muted)' }}
                >삭제</button>
              </div>
            )
          ...
          {filtered.length === 0 && !input.trim() && (
            <p className="px-3 py-2 text-xs" style={{ color: 'var(--text-muted)' }}>카테고리를 입력하고 엔터를 누르세요</p>
          )}
        </div>
      )}
    </div>
